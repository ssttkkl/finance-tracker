...
- [x] 确认关系操作：新增、编辑、解除、驳回、稍后确认；关系不级联删除实际流水。
- [x] 确认用户界面使用「流水记录」和「关联记录合并」，不直接暴露投影、幂等、真删除和校准覆盖等内部术语。
- [x] 确认金额只决定汇总方向：负数为支出、正数为收入、零金额不进入两者；任何金额都保留使用者选择或来源确定的流水类型，“个人转账”只作为关联类型。
- [x] 确认删除有关联流水时由系统在一次确认后自动处理关联并物理删除当前流水，不要求使用者预先逐条取消关联。
- [x] 确认取消关联复用当前 `rejected(user_unlinked)` 决定阻止自动重新匹配，不新增关系版本或抑制表。
- [x] 确认关系重建只写派生投影，不能反向覆盖实际流水当前字段；合并条目稳定取自主记录。
...
- [x] 将方案 C 收敛为唯一原型，补齐流水类型、零金额、关联记录查看 / 编辑 / 添加 / 取消和删除影响确认。
- [x] 将数据库账户支持币种、手工流水币种下拉和导入未支持币种处理补入同一原型，并保持既有信息抽屉风格；不增加账户设置页。
- [x] 将用户可见术语收敛为「收支详情」「流水类型」「关联记录」「待确认」，其中「待确认」只用于系统扫描或独立审查结果；错误状态不再提示 API、数据库或工作区配置；原型和生产 UI 保持同一套词汇。
...
- [x] 本轮产品回流：收支列表改为分页器切换；关联检索增加可调整时间范围，默认覆盖当前流水日期前后三天；候选行用左侧竖线表示选中态并移除 radio。
...
- [x] 为普通字段编辑后重建不改写 `cash_transactions`、稳定主记录和非主记录详情保留补投影回归测试。
...
- [x] 为收支列表分页器的首次打开、上一页、下一页、翻页失败和筛选回到第一页补 Web 回归测试。
...
- [x] 增加 Web 手工表单、校准编辑抽屉、关联记录查看 / 编辑 / 添加 / 取消、删除影响确认和投影刷新契约。
...
- [x] 实现收支流水列表的分页器切换，移除滚动触底和「加载更多」追加。
...
- [x] 设计复核：流水类型、零金额、关联记录管理、删除影响、关系待审、空 / 错误 / 成功状态、无障碍和 4 个目标宽度均覆盖。
...
- [ ] 补充一次可复现的性能基线或范围化性能检查；当前功能正确性和响应式已验证，但尚未记录加载耗时、资源体积或写入耗时基线。
...
- [x] 补跑列表分页、时间范围和无 radio 候选选择的定向测试、完整 Web 测试、视觉回归和 `.ft` SQLite 桌面 / 移动端浏览器 QA；本轮 Vitest 46 passed，构建通过，Playwright 主流程 11 passed，视觉回归 12 passed；真实 SQLite QA 覆盖桌面 / 移动端列表分页、关联默认日期、候选分页、整行选中态和无横向溢出。
...
- [x] UI 原型确认后，回写本任务中的信息架构、状态覆盖和响应式证据；方案 C 采用卡片流，复用既有信息抽屉切换查看态与编辑态，并在同一抽屉管理关联记录。
...
- [x] 本轮流程反思：分页器表达的是页面切换，候选筛选表达的是范围收窄；两者都不能用无界列表追加掩盖当前位置或筛选边界。
...
- **真实 `.ft` SQLite 浏览器 QA**：2026-08-09 CST，以 `127.0.0.1:5174` 前端连接 `127.0.0.1:8766` 后端，在 1440 × 900 和 390 × 844 检查列表分页、筛选回第一页、关联默认日期 `2026-06-30` 至 `2026-07-06`、候选分页与整行选中态；请求包含 `date_from` / `date_to` / `cursor`，未预取前无候选请求，无保存方式 / 稍后确认 / 新建入口，页面无横向溢出，控制台错误为 0。
