...
系统 MUST 为东方证券、IBKR、Charles Schwab 与盈立证券每条由投资账单导入的来源业务行在 `source_payload` 保存完整可归属的原始列值或原始文本单元，不得以归一后的 `record_type`、`record_subtype`、中间 `DEPOSIT` / `WITHDRAW`、标的、金额、费用、币种、账户、幂等身份或编排字段替换、补充原始数据。该限制针对解析和映射生成的字段；来源 CSV 自身具有同名列时，仍必须保留原始列名和值。具有唯一表头的 CSV 必须保留完整原始行（含空列）；多区段 CSV 与 PDF 必须保留该行的全部原始文本单元。由账单汇总生成的 `snapshot` 没有来源动作时，快照 MUST 保存可归属的原始汇总单元；由期末持仓缺席推导的零持仓快照 MUST 保存期末持仓汇总的原始文本单元，不得伪造 `action_raw`、旗标、`CHECKIN`、标的、数量或日期字段。导入编排 MUST 只持久化解析器显式提供的私有来源快照；缺失或无效快照 MUST 阻断整批导入。对于东方证券 PDF，来源行快照 MUST 保留“银行转证券”“证券转银行”“OTC资金划入”“OTC资金划出”“利息归本”与“股息红利差异扣税”中的原始动作。用户明确授权重建特定账本时，系统 MUST 先创建可恢复备份，再从原始账单重新发布投资事件；不得以直接更新既有正式事实伪造来源行或改变其语义。
...
#### Scenario: 投资来源快照不混入标准行
- **WHEN** 任一现有投资导入器将来源业务行映射为投资事件
- **THEN** 该事件的 `source_payload` MUST 只包含原始来源列值或原始文本单元；不得新增解析器的 `action`、`flag_norm`、`kind`、`ticker`、`amount`、`record_type`、`record_subtype`、`source_type`、`record_id`、账户名、产品档案或以下划线开头的编排键。来源文件自身具有同名列时，必须原样保留该列
