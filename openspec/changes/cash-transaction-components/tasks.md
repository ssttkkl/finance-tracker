...
- 修复提交 `3ecc95c` 及本记录已推送；本次验证回写前分支 HEAD 为 `99014f4`，比较基线为 `origin/refactor/web=48a733f`。本地完整 performance 矩阵 `env FT_TEST_POSTGRES_URL='<redacted>' FT_REQUIRE_TEST_POSTGRES=1 PYTHONPATH=tests:.:src uv run pytest -m performance -q --tb=short` → `52 passed, 1716 deselected, 2 warnings`，耗时 `619.87s`。
- PR 检查重跑 `35575381868` / `35575381871` 已全部通过：Backend Performance `18m06s`、Backend PostgreSQL `15m47s`、Backend SQLite `4m30s`、Frontend Web `2m55s`、Shared and JavaScript `1m16s`、Android Debug APK `11m26s`、iOS Simulator app `18m00s`。文档证据提交后的最新 required checks `35577028578` / `35577028526` 也已全部通过：Backend Performance `19m01s`、Backend PostgreSQL `9m11s`、Backend SQLite `4m30s`、Frontend Web `3m09s`、Shared and JavaScript `1m15s`、Android Debug APK `16m01s`、iOS Simulator app `18m26s`。本节更新前 HEAD 为 `d9d2f13`，下一步仅剩合入 `refactor/web` 并回写最终 merge commit、检查 URL 和目标分支 HEAD。原有 PostgreSQL 财富 100k 冷重建 p95 宿主机抖动仍作为独立环境残余风险记录，不属于本次 cash projection 修复范围。
- 最新 HEAD `27d712f` 的检查 `35595893946` 首次仅在 PostgreSQL `test_fixed_10k_cash_related_delete_modes_meet_100ms_budget` 失败：`related_key_edit` p95 `100.465920ms`，其余 `51` 个性能用例通过；该用例只超预算约 `0.47ms`，与同一代码的前后两轮全绿结果一致，判定为 runner 抖动。使用 `gh run rerun 35595893946 --failed` 重跑失败 job `106326188273`（attempt 2）后通过，耗时 `18m59s`；最终 7 个 required checks 全部通过。文档更新前 HEAD 为 `27d712f`，下一步仅剩合入 `refactor/web` 并回写最终 merge commit、检查 URL 和目标分支 HEAD。
