...
- 工作区认证壳只负责会话、工作区选择、创建、成员和邀请流程。已有活动工作区时必须挂载唯一的 `App` 账本路由壳，而不能直接挂载旧 `CashLedgerPage`；该路由壳统一处理收支账本、分类管理、导入和投资账本路径。所有投资 API 请求和投资组合事件流与收支账本一致，允许精确 HTTPS API 来源并携带工作区会话 Cookie。
