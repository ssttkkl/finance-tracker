...
- [x] 6.4 运行 `openspec validate --all --strict`、`openspec doctor`、`git diff --check`、受影响 Python/TypeScript/Vitest 测试、类型检查和生产构建；CI job `35985217557` 失败后补跑 Web E2E 并同步过时的批量导入夹具。
...
- **CI 失败修复（2026-09-24）**：GitHub Actions job `35985217557` 的 7 个 Web E2E 失败均由测试仍在选择文件后直接等待「映射账户」造成；批量扫描夹具同时缺少 `import_token`，加密 PDF 夹具仍使用旧的单文件密码头。更新 `web/tests/cash-ledger.e2e.ts` 后，`npm run test:e2e --workspace finance-tracker-web -- tests/cash-ledger.e2e.ts` → `18 passed`，`npm run test:web` → `15 files / 152 tests`，`npm run build --workspace finance-tracker-web` 通过；加密 PDF 场景验证首次批量扫描返回文件级 `password_required`、二次请求使用 `X-FT-Statement-Passwords`，并且返回已识别文件后不重复扫描。复核时基线 `HEAD` 为 `9211dda`，工作分支为 `codex/pr-90-login-api-host`。
