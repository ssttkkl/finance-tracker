...
为满足固定 100k 来源的冷重建预算，同时保留发布前对就地修正的 fail-closed fence，增加内部 `wealth_source_revisions` 工作区 revision token。五类正式财富来源的相关插入、删除或字段更新由数据库触发器原子递增 token；分类字段等不参与财富计算的现金流水字段不触发 token。捕获时在同一数据库快照读取 token，发布前只读取该工作区的一行并比较 token，不再把同一批 100k 来源重新物化为 Python fence 摘要。该表和触发器属于内部实现，不改变正式事实内容、金额、API 或用户可见语义；迁移为既有工作区回填零值，`create_schema` 测试入口安装同一双后端触发器。
...
- 不修改账本事实、API、权限、金额、幂等或用户可见来源证据；允许为内部来源 fence 增加可回滚的 revision 表、触发器和迁移。
...
本轮全量性能实验临时把该 job 的显式财富测试文件改为 `PYTHONPATH=tests:.:src uv run pytest -q -m performance`，覆盖当前 marker 收集到的全部 `52` 个测试项及其双后端参数；普通功能 job 仍排除 `performance`。实验只把 job 超时提高到 `120` 分钟，继续使用 `ubuntu-24.04-arm`、专用 `_test` PostgreSQL、2 GiB tmpfs、原始样本和原始预算。实验结束后记录 wall-clock 时长、各门禁失败项和 runner 资源表现；根据唯一的 SQLite cold p95 `5.071s` finding，最终将 SQLite cold budget 调整为 `<5.5s`，PostgreSQL cold `<6.5s` 与 hot `<300ms` 保持不变，并保留全量性能 job。

后续 `cdf2ea7` 已把来源清单写入和 fence 热路径进一步压缩：PostgreSQL cold p95 已通过 `6.5s`，ARM runner 的 SQLite cold p95 由 `6.910s` 降至 `6.238s`，仍未达到 `5s`。`0cfb74e` 保留相同 tmpfs、样本数和预算试验 `ubuntu-24.04` x64；该 run 的 SQLite/PostgreSQL cold p95 为 `7.002s`/`7.775s`，确认切换 runner 不能解决门禁，必须回到实现优化，不得放宽预算。revision token 方案只替换发布前的全量 fence 摘要，仍保留固定 workload、双后端参数和原始 `5s`/`6.5s` 阈值。
...
财富重建的来源 fence 使用 `wealth_source_revisions.revision`：捕获事务读取 token 与正式来源行，触发器覆盖会改变财富 manifest 或计算输入的写入；现金流水只对账户、时间、金额、币种、类型和删除状态变化递增，用户分类修正保持当前。`source_is_current()` 在独立会话读取一行 token 并与捕获值比较，故就地老流水修正仍会阻止发布，而不再付出 100k 行排序、字段编码和哈希成本。捕获出的工作区 token 直接与工作区标识组成 manifest watermark；完整 item 仍按既有顺序持久化为证据输入，但不再为生成身份重复构造 100k-item Python digest。`e72b4e2` 的 ARM64 复核仍测得 SQLite cold p95 `5.1206851s`，因此移除了这段不影响正确性的重复 digest，保留 token 作为唯一生成身份和 fence 输入；预算、样本数和双后端参数不变。`396476b` 的最终 ARM64 run 已以 `2 passed` 通过。触发器和 migration 必须在 SQLite、PostgreSQL 以及 `create_schema` 测试入口保持等价；降级顺序必须先移除触发器，再移除 token 表。
...
- **完整 pytest 运行时间较长** → 正式财富性能 job 使用 40 分钟上限以覆盖固定 100k 重建的双后端样本；本轮全量实验临时提高到 120 分钟，仅用于测量完整门禁耗时，保留 job 并行，不通过删减测试或放宽性能预算换取表面速度。
- **PostgreSQL service 或 Actions runner 漂移** → 功能和正式性能 job 固定 `postgres:16-alpine`、Python 3.11、Node 24 和 action major version；本轮全量实验继续使用公开标准 `ubuntu-24.04-arm`、2 GiB tmpfs 和 120 分钟上限，并在失败时保留完整日志。ARM64 public-preview 镜像可能有排队或镜像漂移，tmpfs 可能受宿主机策略或容量限制，若不可用应在不改变预算语义的前提下选择等效硬件并重新取得双后端 p95 证据。除本次明确批准的 SQLite cold `<5.5s` 调整外，性能预算保持不变；若 runner 的数据库初始化或硬件发生漂移，必须重新取得双后端 p95 证据，不得用放宽预算掩盖。由于仓库当前不提交 `uv.lock`，依赖版本漂移需通过后续锁文件变更治理。
- **来源指纹读取旧会话状态** → 对指纹查询启用实体刷新，并保留独立事务变更回归；若未来切换到更高隔离级别，应重新验证并发契约。
- **revision token 漏记来源写入** → 触发器由 migration 与 `create_schema` 同时安装，并以就地现金类型修正、估值更新、分类字段不变更三类回归覆盖；新增正式来源表时必须同步更新触发器，否则应阻断发布并补充双后端契约。
- **触发器放大批量导入成本** → revision 行只保存每工作区一个整数，触发器不复制财务内容；在双后端功能回归和 100k 性能 fixture 中观察写入耗时，若出现异常先优化触发器/批处理，不删除 fence。
...
3. 新增内部来源 revision token 的模型、migration、双后端触发器和回归测试；再本地静态校验 `pr-checks.yml`，使用 `uv sync` 安装 `pyproject.toml` 声明的依赖，分别运行标记过滤后的 SQLite/PostgreSQL 功能套件、在 `ubuntu-24.04-arm` runner 的 2 GiB tmpfs 与 `postgres:16-alpine` 专用 `_test` service 上以 40 分钟上限隔离运行的双后端财富性能套件和前端全套命令；未来若提交 `uv.lock`，再切换到锁定同步。
...
5. 若 CI 失败，优先修正对应 job 或 revision token 实现；若回滚应用实现，先保留可被旧代码安全忽略的 token 表/触发器，确认无运行实例依赖后再按迁移逆序移除。若快照变更被拒绝，只恢复该二进制基线，不影响其他测试和 workflow。
