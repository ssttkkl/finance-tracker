...
- **PostgreSQL service 或 Actions runner 漂移** → 功能 job 固定 `postgres:16-alpine`，性能 job 固定 `macos-26-xlarge`、Homebrew PostgreSQL 16、Python 3.11、Node 24 和 action major version，并在失败时保留完整日志；larger runner 可能受组织配置、排队和计费策略影响，若不可用应在不改变预算语义的前提下选择等效硬件并重新取得双后端 p95 证据。性能预算保持不变，若 macOS runner 的数据库初始化或硬件发生漂移，必须重新取得双后端 p95 证据，不得用放宽预算掩盖。由于仓库当前不提交 `uv.lock`，依赖版本漂移需通过后续锁文件变更治理。
...
3. 新增并本地静态校验 `pr-checks.yml`，使用 `uv sync` 安装 `pyproject.toml` 声明的依赖，分别运行标记过滤后的 SQLite/PostgreSQL 功能套件、在 `macos-26-xlarge` runner 专用 `_test` PostgreSQL 上以 40 分钟上限隔离运行的双后端财富性能套件和前端全套命令；未来若提交 `uv.lock`，再切换到锁定同步。
