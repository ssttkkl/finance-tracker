...
本轮全量性能实验临时把该 job 的显式财富测试文件改为 `PYTHONPATH=tests:.:src uv run pytest -q -m performance`，覆盖当前 marker 收集到的全部 `52` 个测试项及其双后端参数；普通功能 job 仍排除 `performance`。实验只把 job 超时提高到 `120` 分钟，继续使用 `ubuntu-24.04-arm`、专用 `_test` PostgreSQL、2 GiB tmpfs、原始样本和全部预算。实验结束后记录 wall-clock 时长、各门禁失败项和 runner 资源表现，再决定是否拆分长期 job；该临时超时不代表放宽任何产品性能预算。
