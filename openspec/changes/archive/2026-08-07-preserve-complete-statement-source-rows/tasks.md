...
- [x] 5.1 运行新增回归测试、受影响导入与迁移测试、真实 PostgreSQL 契约矩阵及完整 Python 回归。
...
- [x] 8.1 将已验证的 delta 规格同步到 OpenSpec 主规格，复核同步结果后归档变更。
...
- 归档前完整回归：在 `HEAD=c66a66f747ca4d4ddcd7f92af78f046b86483029` 运行 `uv run pytest -q`，结果为 `1265 passed, 142 skipped, 1 warning`，耗时 `233.09s`；唯一 warning 为测试依赖的 Starlette 弃用提示。
- 归档前 delta 同步复核：逐一检查 009、013 与 015 的 requirement 和 scenario 标题，主规格已包含全部新增或修改意图；保留后续 change 已收紧的 `counterparty_account_attrs` 合同，不以较旧 delta 覆盖。
