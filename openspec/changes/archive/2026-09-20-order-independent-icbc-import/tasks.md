...
- [x] 5.2 运行 SQLite 受影响矩阵、构建、类型/编译检查、`git diff --check` 和严格 OpenSpec 校验；受影响测试通过，完整回归的已知基线失败见最终验证记录
- [x] 5.3 使用名称以 `_test` 结尾的专用 `FT_TEST_POSTGRES_URL` 补跑同一导入/关系合同矩阵，结果见最终验证记录
...
- PostgreSQL：首次验证时环境未配置 `FT_TEST_POSTGRES_URL`；后续已按补跑条件配置名称以 `_test` 结尾的专用数据库，最终结果见第 9 节。
...
- [x] 7.4 在信用卡/借记卡解析输出中保存完整或 PDF 明确稳定掩码来源身份、尾号字段、渠道和稳定业务行标识；借记卡身份只来自文件顶部 `卡号`，缺失稳定身份时失败关闭，不增加历史工行身份兼容回退
...
- [x] 9.1 根据真实 PDF 复核并更新范围：顶部 `卡号` 是文件级本方卡号，表格 `账号` 不得作为卡号或来源账户身份；不增加旧数据兼容回退
- [x] 9.2 先增加失败回归：声明卡号贯穿所有明细、表格账号不得覆盖但仍参与业务行去重、缺失或多卡号失败关闭
- [x] 9.3 修改文本/表格两条借记卡解析路径，统一传递文件级卡号并输出卡号尾号；保持来源行快照中的表格原始列不变，并用表格账号区分原始业务行而不改变来源映射
- [x] 9.4 重新运行信用卡、借记卡及含建行账单的正反顺序隔离导入验证，并记录事实、关系、投影和来源身份结果
- [x] 9.5 完成 SQLite 回归、OpenSpec、构建、差异检查、适用 PostgreSQL 矩阵和当前 diff 独立复核

### 9.4–9.5 验证记录

- 真实借记卡解析：使用仓库外 `~/.ft/bills` 中的工行借记卡 PDF，表格解析得到 `1206` 条记录、`1` 个来源身份，来源卡号尾号和输出 `card_number` 均为 `3697`；每条记录的来源身份与来源行快照中的表格 `账号` 均不同。文本回退解析同样提取到尾号 `3697`。未输出密码、完整卡号或文件名。
- 真实正反顺序重放：在两个全新隔离 SQLite 库中导入 `3` 份微信、`3` 份支付宝、`2` 份工行信用卡、`1` 份工行借记卡和 `2` 份建行借记卡。正序与逆序均为 `11390` 条事实、`490` 条活动关系、`10907` 个投影、`11390` 个投影成员，投影状态均为 `ready/succeeded`；事实、关系和投影摘要完全一致。工行借记卡导入 `1206` 条、跳过 `0` 条；建行借记卡导入 `952` 条、跳过 `0` 条。加入表格 `账号` 参与事实去重的修正后已重新运行，结果不变。
- SQLite 受影响矩阵：`PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m pytest -p no:cacheprovider tests/test_mapping.py tests/test_convert.py tests/test_complete_statement_source_payload.py tests/test_statement_account_mapping.py tests/test_cash_import_wizard.py tests/test_transaction_relations_payment_mirror.py tests/test_transaction_relations_refund.py tests/test_transaction_relations_cross_batch.py tests/test_import_relation_planning.py tests/test_transfer_phase_c.py tests/test_transaction_relations_transfer.py tests/test_source_agnostic_transfer_matching.py` → `417 passed, 13 skipped`；补跑 `tests/test_postgres_statement_import.py` → `25 passed`。
- 完整回归：`PYTHONDONTWRITEBYTECODE=1 .venv/bin/python -m pytest -p no:cacheprovider` → `1568 passed, 192 skipped, 1 failed`；唯一失败仍为既有 `tests/test_application_queries.py::test_list_accounts_values_cash_and_investment_with_cost_fallback`（实际 `Broker=25`，期望 `29`），与本次工行解析改动无关。
