...
- [x] 2.2 先添加投资导入与重放失败测试：利息、税费、外汇净额、奖励和出金冲回不得生成 `funding(external)`；外部出入金与内部子账户调拨必须得到 `external` 与 `subaccount` 两种不同子类型。
...
- [x] 3.2 在投资领域集中定义并验证投资事件记录类型与记录子类型；仅 `funding` 接受 `external` 或 `subaccount`，其他记录类型拒绝不合法子类型。
...
- [x] 7.3 运行 `openspec validate --all --strict`、`openspec doctor`、完整回归、相称构建和 `git diff --check`；对未运行项目记录不适用理由、残余风险和补跑条件。严格校验、构建和差异检查已通过；功能完整回归在 SQLite 与真实 PostgreSQL 均通过；独立财富性能门槛的既有双后端失败已记录。
...
- [x] 9.1 更新领域词表、proposal、delta 规格、设计和任务，固定 `funding`、`trade`、`income`、`expense`、`reversal`、`subscription`、`adjustment`、`snapshot` 及其子类型，声明方向由资产组成表达。
- [x] 9.2 先添加失败测试，覆盖全部类型约束、历史折叠东方证券行的安全降级、各导入器的利息/税费/冲回/内部调拨语义、投资收益与费用归集，以及只有 `funding(external)` 可配对。
- [x] 9.3 改造领域类型、模型约束、迁移、投影、财富事实、运行时查询和命令，使旧一级类型不再可写或可读。
- [x] 9.4 改造东方证券、IBKR、Schwab、盈立与连接器导入器，保留原生动作并基于结构化字段生成经济事实类型与子类型。
- [x] 9.5 在 SQLite 与真实本机 PostgreSQL 上运行迁移、导入、关系、投影与财富事实矩阵；更新主规格、审查记录和发布/回滚说明。
...
- 产品与范围复核：通过。自动扫描仅接受 `funding(external)`；方向由资产组成判断。`funding(subaccount)`、利息、税费、外汇净额、奖励、出金退款和未分类现金调整均不能进入候选。
...
- SQLite 完整功能回归：`uv run pytest -q --ignore=tests/test_wealth_performance.py`：`1234 passed, 130 skipped`，135.69 s。新增回放回归测试确认非法 `record_subtype` 在创建任何账户或持仓前失败，且不修改快照。
- 本机 PostgreSQL 完整功能回归：`FT_TEST_POSTGRES_URL='postgresql+psycopg:///finance_tracker_test' FT_REQUIRE_TEST_POSTGRES=1 uv run pytest -q --ignore=tests/test_wealth_performance.py`：`1381 passed, 10 skipped`，311.69 s。
- 独立财富性能门槛：`FT_TEST_POSTGRES_URL='postgresql+psycopg:///finance_tracker_test' FT_REQUIRE_TEST_POSTGRES=1 uv run pytest tests/test_wealth_performance.py -q`：两个后端参数均失败，耗时 410.20 s。SQLite 冷重建 p95 为 `5.842227166 s`，超过 `5 s` 门槛，热读 p95 为 `0.061066541 s`；PostgreSQL 冷重建 p95 为 `10.066638750 s`，超过 `6.5 s` 门槛，热读 p95 为 `0.077033458 s`。该夹具仅将 50 条种子投资事件从旧 `buy` 替换为等义 `trade(security)`，且冷重建结果低于此前记录的 `6.899861208 s` / `14.162753041 s`，判定为既有财富事实重建性能风险；不得通过放宽门槛掩盖此风险。补跑条件为完成财富事实重建优化后，在同一命令下完整复跑。
