...
- [x] 1.2 完成 `grill-me`/`grilling` 澄清：Android 与 iOS 一起改；使用 Release-like 测试包，Android 使用仓库内非生产测试 keystore、iOS 改为面向 `iphoneos` 的未签名 `.ipa`；不设置 `EXPO_PUBLIC_FT_API_ORIGIN`，由登录时输入 API origin；测试包允许带显式端口的 HTTP；CI 只生成 artifact。
...
- [x] 3.2 复核测试断言只检查用户确认的 CI 产物合同：Android 使用仓库测试 keystore、iOS 生成 `iphoneos` 未签名 `.ipa`，不把具体 Expo/Gradle/Xcode 内部实现写成不必要的公共契约。
...
- [x] 5.3 完成最终 diff 复核；确认 Android 使用测试密钥、iOS 生成未签名 `iphoneos` `.ipa`、artifact 内置 JavaScript、登录地址控件开启且空地址不会发出网络请求。
...
- [ ] 6.4 在可用本机工具链上复现 Android Release 与 iOS `iphoneos` Release 构建；检查 Android APK 存在内置 JS bundle 且使用仓库测试证书，iOS 未签名 `.ipa` 存在标准 `Payload/*.app` 和内置 JS bundle；Android 在无 Metro 时完成可用的 Native 入口验证，iOS 在外部签名后补做真机入口验证。
...
- [x] 8.1 记录可复用的 Expo Native CI bundling、登录时 origin 选择、空构建地址、HTTP 测试地址、Android 测试签名和 iOS `iphoneos` 未签名 `.ipa` artifact 约定。
...
- **工程/安全复核**：已完成；确认 workflow 不读取 `EXPO_PUBLIC_FT_API_ORIGIN`、账号或生产签名 Secret；空 origin 在登录提交前失败；HTTP 只在显式开启地址覆盖且带端口时允许；Android 只使用仓库测试 keystore，iOS 只生成面向 `iphoneos` 的未签名 `.ipa`，不读取 Apple 签名凭据。
...
- **Native QA**：Native 单元测试与无构建地址的 Android/iOS Expo export 已通过；真实 Android Release APK、iOS `iphoneos` 未签名 `.ipa`、无 Metro 启动和设备登录流程仍由 6.4/6.6 保留为未完成项，不宣称通过。
...
- **本次 iOS IPA 增量验证**：先更新静态契约测试并确认旧模拟器 workflow 下按预期失败（2 failed / 2 passed），再运行 `PYTHONPATH=src uv run --no-sync pytest -q tests/test_mobile_ci_packaging.py`（4 passed）、`ruby -e "require 'yaml'; YAML.load_file('.github/workflows/mobile-ci.yml')"`、`git diff --check`、`openspec validate --all --strict`（35 passed）和 `openspec doctor`，均通过；未在本机执行 Xcode `iphoneos` 构建。
- **本次增量基线与时间**：修改前 `HEAD` 为 `785a22c1aaad5e2be3b212c47ff75d8f4a00aac2`（`fix: 支持 Native 测试包登录时指定后端地址`）；记录时间为 2026-09-24 17:50 +0800。
...
- **未完成**：本次未重新运行 Android `:app:assembleRelease`、iOS `pod install`/Xcode `iphoneos` Release、无 Metro 启动或真实设备登录。PR 原有记录显示本机 Maven/React Native 依赖下载曾阻断这部分验证；需在依赖可用的 runner 或本机缓存完成后补跑 6.4/6.6，不能据此宣称 APK 签名、iOS `.ipa` 内置 JS、无 Metro 启动或登录流程已通过。
- **补跑条件**：在能稳定访问 Maven/React Native 依赖的 runner 或本机缓存完成后，重跑 `6.4` 的 Android Release 与 iOS `iphoneos` Release 构建、IPA `Payload`/bundle 检查、Android 签名指纹检查和适用的无 Metro 启动验证；iOS 真机启动还需要外部签名。该条件不影响 workflow 的实现和测试 keystore 提交，但阻止本变更归档为全部验证完成。
