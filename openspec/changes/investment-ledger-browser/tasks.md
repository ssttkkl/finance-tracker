...
- [x] 4.33 修复认证工作区运行时遗漏的投资服务装配：分别绑定投资事件、持仓与刷新协调器到活动工作区，避免复用收支代理、空持仓服务或后台线程丢失工作区上下文。
...
- [x] 5.32 走查认证运行时全部服务装配与跨 Service 客户端边界。发现并修复投资账户错误走收支代理、投资事件缺少查询服务、持仓/刷新服务为 `None`、以及刷新线程不能依赖请求 `ContextVar` 四项问题；未发现其他生产 Web API 存在同类空装配或错误工作区代理。
...
- [x] 6.32 运行工作区投资运行时回归：`uv run pytest tests/test_user_workspace_access.py tests/contract/test_investment_web_api.py tests/test_application_investment_web_queries.py tests/test_cli.py -q` → 53 passed、4 skipped；以本地 SQLite 运行时注册并创建独立工作区后，投资账户、事件与 `phase=holdings` 分别返回 200 和该工作区的空结果；`git diff --check` 通过。`FT_TEST_POSTGRES_URL` 当前未设置，补跑条件是设置指向 `_test` 数据库的连接串后重跑同一契约矩阵。
