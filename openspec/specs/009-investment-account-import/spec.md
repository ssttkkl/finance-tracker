...
- - **FR-003**: 系统 MUST 使用 `record_type` 和 `record_subtype` 持久化投资事件的规范语义，只允许 `funding(external|subaccount)`、`trade(security|fx|repo)`、`income(dividend_cash|dividend_stock|interest|reward)`、`expense(commission|tax|interest|handling_fee|penalty)`、`reversal(expense_tax|expense_interest|expense_commission|funding_withdrawal)`、`subscription(ipo_debit|ipo_refund)`、`adjustment(fx_net|manual|unclassified)` 与 `snapshot(cash|position)`。每种类型 MUST 有明确的快照应用逻辑；方向只由 `from_*` 与 `to_*` 资产组成表达。
