# Tasks: 导入时生成现金流水标准记录类型

**Input**: `spec.md`、`plan.md`、`research.md`、`data-model.md`、`contracts/record-type.md`

**变更等级**: A 类完整 Feature。原因：新增持久化字段、改变导入数据合同和财务业务分类。
...
- [X] T004 完成 Spec Kit 一致性分析，确认 FR/SC/任务覆盖且无 CRITICAL/HIGH；2026-08-01，`check-prerequisites.sh --json --require-tasks --include-tasks` 通过，人工复核产物链路覆盖分类、持久化、双后端、重建和回滚。
...
- [X] T019 完成范围化差异核对、`git diff --check`、Spec Kit artifact 一致性复核和收敛检查；当前实现覆盖 spec 中的导入分类、持久化约束、SQLite 迁移、双后端契约与全量重建。Python 全量回归已通过；未解决风险为 Web 构建仍受既有 `tests/CashTable.test.tsx:37` 参数错误阻断，真实 PostgreSQL 契约因未提供 `FT_TEST_POSTGRES_URL` 未执行。

## 非目标

- 本 Feature 不修改任何关系配对 Phase、关系表内容或 `record_type` 之外的投影规则。
- 本 Feature 不提供旧数据库兼容读取、历史记录回填或旧关系重建。
...
验证补充（2026-08-02）：`uv run pytest -q` 为 `1098 passed, 108 skipped, 1 warning`；
`uv run python -m compileall -q src tests` 通过；`git diff --check` 通过。真实 PostgreSQL
仍因未设置 `FT_TEST_POSTGRES_URL` 跳过。主库 schema 为 `20260802_15`，`PRAGMA integrity_check`
返回 `ok`；重建前备份为
