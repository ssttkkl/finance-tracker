...
- Q: 审计发现？ → A: 支付宝提现→工行 **6/6** accepted；建行跨卡/借记还信用卡多组正确；**微信零钱提现 4/4 未挂上**；**credit_repayment pending 噪声**（京东「还款」摘要、月付转出流水配到无关小额入账/退款）。
...
- Q: 信用还款如何收紧？ → A: 转出流水 MUST 含明确还款信号（信用卡还款/购汇还款/自动还款/花呗*还款/月付】主动还款等）；转入流水 MUST 为 **loan/credit 账户** 或信用卡侧「还款/转帐收入」且 **禁止** 以退款/消费退货/营销刷卡金/小额商户入账为对侧；建行 summary 仅「还款」+ 商户 counterparty（如京东）MUST NOT 当作 credit_repayment 转出流水。多候选或金额显著不等（非 FX 规则）→ pending/open-leg 或 skip，不得乱配。
- Q: 购汇还款多候选如何配？ → A: 拉取转出流水业务日（Asia/Shanghai）市场中间价；对每个 loan 入账候选算隐含汇率偏差 `rate_error`；**仅当恰好 1 个**高置信候选（默认 rate_error≤1.5% 且显著优于次优）auto-accept；汇率不可得或置信不足/并列 → pending（见 `attachments/transfer-source-taxonomy.md` FX 表与 006 FR-018）。禁止无汇率时把任意 ≤10s loan 入账 auto-accept。
...
- Q: 哪些算 transfer？ → A: **自有账户调拨**（提现到银行卡、卡间银联、平台内余额宝/余利宝到卡、银证）、**信用还款**（信用卡/花呗/月付）。**不是**：红包/二维码收付款/闲鱼转账等强 P2P、商户消费、退款、通道消费（走 mirror）。
- Q: 真实数据锚点？ → A: 支付宝提现→工行 6/6 秒级；建行跨卡支取↔银联入账多组；微信提现已到账↔建行入账（常 date-only）；银证常仅有一侧流水。
...
### Session 2026-07-22 — transfer 排除分层（强 P2P vs 软平台转账）

- Q: 所有含「转账」的 P2P 是否一律禁止 `transfer_pair`？ → A: **否，必须分层**。
- Q: **强排除**（任一侧命中 MUST NOT 进入 `transfer_pair` 自动池，亦 MUST NOT auto-accept）？ → A: **红包**（微信红包/红包单发/群红包等）、**二维码收付款**（二维码收款/扫二维码付款/收款方备注等）、**闲鱼转账**、**群收款**。语义上不可能是自有账户镜像；「闲鱼转账」为 P2P 收款/income，绝非自己转自己。
- Q: **软处理**（外观像 P2P，但可能是微信/支付宝内自有余额→自有银行卡）？ → A: **微信转账**、**支付宝转账**、以及常与之绑定的 **转账备注** 字段。MUST NOT 与红包同级强排除。
- Q: 软处理如何配对？ → A:
  1. MUST NOT 仅因含「转账/微信转账/支付宝转账」而对 **双侧均为平台零钱/余额、且无提现·到卡·银联入账等自有调拨证据** 的边 auto-accept；
  2. 当对侧为 **银行入账**（或转出流水为明确提现/转出到银行卡）时 MAY 进入候选；默认 **pending_review**，仅当命中既有 withdraw_to_bank / 银联入账等强规则时 MAY auto-accept；
  3. MUST NOT 用裸「闲鱼」「转账」作强排除短语（避免误伤闲鱼寄件/分账与真·银行转账文案）。
...
- Q: 微信确实只有一侧流水？ → A: 本库 0；误判为缺少对侧流水来自时间窗/拆退/状态白名单过窄。
...
**Independent Test**: 同批含支付宝退款、银行通道入账、消费退货；check 后平台退款边 rule 属 Phase A；mirror 边存在，且不会把已关联退款连到错误流水。
...
- **FR-043**: transfer 候选排除 MUST **分层**：
  - **强排除**（硬禁止进 `transfer_pair` 自动池 / 不得 auto-accept）：二维码收付款（含扫二维码付款、收款方备注）、**闲鱼转账**、群收款、红包（微信红包/红包单发/群红包等）、商户消费、退款/消费退货、通道消费（走 mirror）。
  - **软处理**（不得与红包同级硬排除）：**微信转账**、**支付宝转账**、**转账备注**。此类 MAY 与银行入账/提现到卡路径候选；双侧均为平台 P2P 文案且无自有调拨证据时 MUST NOT auto-accept；有提现/转出到银行卡/银联入账等强证据时遵循 FR-044。
  - 仍 MUST NOT 仅因裸「转账」二字进候选（FR-041）；强排除短语 MUST 精确匹配（如「闲鱼转账」），禁止裸「闲鱼」。
- **FR-044**: 精细优先级（唯一 auto）：
  ...
  3. **信用还款（收紧）**：转出流水明确还款信号；转入流水 loan/credit 或信用卡还款/转帐收入；禁止京东等商户「还款」摘要当转出流水；禁止退款/退货/刷卡金当转入流水；
  4. 银证：无对侧 open-leg；
...
- **SC-006**: 关单+全额退款余额净 0。
- **SC-007**: 微信收入退款流水 Phase A 后 **100%** 挂原单（基线 82/82 有对侧；含拆退）。
...
- **SC-020**: 强排除文案 MUST NOT 成为 transfer auto 边：`扫二维码付款` / 红包 / **闲鱼转账** / 群收款；`对方已收钱×转账` / `已存入零钱×转账` 等纯平台 P2P 边 MUST NOT 仅因「转账」字样 auto-accept。
- **SC-020a**: 「闲鱼转账」income 与任意近时同额支出（闲鱼寄件、商户消费等）MUST NOT 产生 `transfer_pair`（pending 或 accepted）。
- **SC-020b**: 仅含「微信转账/支付宝转账/转账备注」且无提现·到卡证据的双侧平台边 MUST NOT auto-accept；含提现/转出到银行卡/银联入账等强路径时仍可按 FR-044 配对。
...
- **FR-057**: 同账户等额 mirror 多候选时 MUST **按评分+时间最近** 取一对 accepted（1–1 greedy 全局仍保证每条流水最多参与一次配对），MUST NOT 仅因 candidate_count>1 降为 pending。建行 date-only 且摘要同为「消费」时，任选唯一配对可接受。
...
- **FR-055**: **Diamond refund**（`refund_offset.diamond_via_platform.v1`，Phase D）：对银行退款流水，若存在唯一链 bank_ref↔plat_ref（mirror）+ plat_ref↔plat_pay（accepted refund_offset）+ plat_pay↔bank_pay（mirror）→ 写 bank_pay–bank_ref `refund_offset` accepted；仅用 accepted 平台退款边；弱 mirror 不得单独支撑菱形角；多 bank_pay → 仍 open-leg。
...
- 不以改金额/删行核销。
- 不把红包/二维码收付款/闲鱼转账/群收款当作自有账户 `transfer_pair`（强排除）。
- 不把「微信转账/支付宝转账」与红包同级一刀切硬排除（可能是自有余额→自有卡；软处理见 FR-043）。
