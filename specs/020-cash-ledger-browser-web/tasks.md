...
- [X] T143 [P] [US5] 先在 `web/tests/CashLedgerPage.test.tsx` 编写失败测试：`invalid_filter` 会标记最低金额和最高金额输入、暴露关联错误说明；修改任一金额字段后无效状态清除；加载、空态和错误态均保留文字，错误态使用左对齐操作布局（FR-023、FR-024、FR-029、FR-038、FR-039、SC-008、SC-009）。
  - 失败证据：2026-08-01，新增断言后 `cd web && npm test` 失败；`最低金额` 未设置 `aria-invalid="true"`。
  - 通过证据：2026-08-01，`cd web && npm test` 为 `25 passed`。
- [X] T144 [P] [US5] 在 `web/tests/accessibility.test.tsx` 或适用前端测试编写失败断言：`web/src/styles.css` 首个非空内容为包含 `macrostructure: Workbench`、`genre: modern-minimal`、`theme: Cobalt` 的标准 Hallmark 标记；筛选 `<summary>` 仍是包含标题和范围摘要的单一整体点击区域（FR-033、FR-038、SC-008、SC-009）。
  - 失败证据：2026-08-01，新增断言后 `cd web && npm test` 失败；旧标记不含 `macrostructure: Workbench`。
  - 通过证据：2026-08-01，`cd web && npm test` 为 `25 passed`。
