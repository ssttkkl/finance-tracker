...
- `web/` 已有高密度表格、筛选、抽屉/全屏详情、键盘焦点和错误状态组件；本次改字段与状态，不重建设计系统。
...
| 前端页面 | `web/src/pages/CashLedgerPage.tsx` 及既有组件 | 收支筛选、投影表格、组成摘要和投影证据。 |
...
右侧抽屉并列显示。列表列优先级为主记录发生时间、账户、交易对方、分类、净额、币种、经济类型、
组成方式、成员数、主渠道和证据入口。
...
```text
Finance Tracker | 收支账本                      | 证据详情
---------------+-------------------------------+------------------------
当前视图        | 日期 / 账户 / 对方 / 分类     | 投影结果
               | 金额 / 币种 / 经济类型        | 主记录
               | 组成方式：单条/同笔/退款/组合 | 全部成员流水
               | 投影条目表 + 分页              | 生效关系 / 未生效提示
               |                               | 退款时间线
```

经济类型使用“全部 / 消费 / 收入”分段控件；组成方式使用选项菜单。内部转账和全额退款没有列表筛选项，
也没有原始流水开关。关系摘要只展示已确认关系；未生效关系只在证据详情提示。

**窄屏**：顶部栏显示产品和“收支账本”；筛选收纳为可展开区域；表格保留时间、交易对方、净额和组成方式；
详情为全屏模态层，关闭后焦点返回原投影条目。触摸目标不小于 44 × 44 px。
...
6. **前端与浏览器**：组成筛选、更新后刷新、无回退请求、证据章节、纯键盘、焦点返回、
   `1440 × 900` 与 `390 × 844` 无重叠。
...
| Design Review | `plan-design-review` | UI/UX | 2（本规格 1） | CLEAR | 8/10 → 9/10；明确桌面并列重排、版本更新时关闭旧证据和既有设计资产复用，0 个未决项；本机缺少设计器 API key，未生成 PNG。 |
