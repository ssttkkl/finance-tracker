...
- Q: 投资账户去掉 currency 身份后报价币种从哪来？ → A: snapshot/事件/metadata.base_currencies；不重做投资产品。
...
作为多币种记账用户，我把“工行借记卡”建成**一个账户**，可以同时持有 CNY、JPY 等币种余额；列表与报告按该账户展示各币种口袋，而不是要求我维护“工行 CNY”“工行 JPY”两个账户行。

**Why this priority**: 这是本 feature 的核心建模变更；没有它，后续换汇/跨币种转账仍会卡在“分册账户”身份上。
...
作为用户，我对多币种账户做 `add` / `checkin` 时，**必须显式指定操作币种**（无账户主币种默认）；系统把金额写入该账户对应币种口袋，不再用“账户自带唯一币种”消歧。

**Why this priority**: 去主币种后，操作币种成为余额口袋键；缺省猜测会导致错账。
...
2. **Given** 账户 `工行` 存在，**When** 执行带显式币种 JPY 的余额校准，**Then** 仅 JPY 口袋被设为校准值，并产生可审计的校准事实/观测。
...
作为导入用户，mapping 仍把账单行解析到**账户名**；行币种直接写入该账户对应余额口袋。**不再**要求预先存在“同名 + 该币种”的分册账户，也**不再**因“行币种 ≠ 账户唯一币种”拒绝现金/贷款账户导入。
...
作为用户，我可以在两个账户之间转账；当转出币种与转入币种不同时，必须提供转入金额；系统用**账户名 + 操作侧币种**指定双方口袋，不再依赖“账户行上的唯一币种”消歧账户身份。
...
1. **Given** 账户 A、B 存在，**When** 从 A 的 CNY 口袋转到 B 的 CNY 口袋并指定金额，**Then** 双方同币种余额按金额增减。
...
作为已有数据的用户/运维，实现完成后通过**一次性迁移**把历史“同名不同币种”账户行合并为单一账户；所有现金事实、投资事件、生命周期、估值观测与导入关联挂到保留的账户身份；合并后系统**不再**提供旧分册语义或兼容查找。

**Why this priority**: 用户明确不要长期兼容层；无迁移则无法在真实数据上使用新模型。

**Independent Test**: 准备同名 CNY/JPY 两行账户及各自事实；跑一次性迁移后仅剩一个账户实体，事实与余额口袋完整；type 冲突样本迁移失败并报出。双后端迁移结果等价。

**Acceptance Scenarios**:

1. **Given** 同 workspace 下存在 `工行`+CNY 与 `工行`+JPY 且 type 相同，**When** 执行一次性迁移，**Then** 合并为一个账户，两币种历史事实均挂到该账户，余额口袋可重建。
...
4. **Given** 迁移涉及估值观测中的现金账户身份，**When** 合并完成，**Then** 现金估值身份可区分同一账户下不同币种，互不覆盖。
...
- 停用/启用、重命名：作用在账户级，影响该账户全部币种口袋的展示与可用性，不按币种分册生命周期。
- security/crypto 账户：展示名在投资账户范围内仍唯一；账户级唯一 currency 身份移除后，投资事件与持仓继续自带 currency/ticker 信息；本 feature 不重做投资产品语义。
- 导入目标账户不存在、mapping 未匹配、digest 幂等、整批事务：保持 004 的失败关闭与幂等合同，仅账户解析键从 name+currency 改为 name。
- 同账户内换汇（CNY 口袋 ↔ JPY 口袋的专用 FX 事件）：**非目标**；若 transfer 指向同一账户名的不同币种，仅作为通用转账路径的自然结果，不定义汇率产品语义。
- PostgreSQL 与 SQLite：账户唯一约束、合并迁移、多币种余额读写的用户可见结果必须等价；禁止自动回退、双写、隐式跨后端迁移。
- 允许的运行差异：锁实现、并发吞吐、错误底层驱动信息；不得造成账务结果或账户身份分叉。

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST 将账户身份定义为 workspace 内 `name` 唯一；账户实体 MUST NOT 再以单一 `currency` 作为身份组成部分。
- **FR-002**: System MUST 允许同一账户持有零个或多个币种余额口袋；余额口袋键为操作/事实上的币种。
- **FR-003**: System MUST NOT 引入账户主币种、默认币种或“代表币种”作为记账缺省来源。
- **FR-004**: 创建账户 MUST 不要求绑定唯一账户币种；若创建时提供可选初始币种，仅可用于初始化该币种 0 余额展示口袋，不得写成账户永久属性。
- **FR-005**: 对手工现金类记账（add）与现金余额校准（checkin），System MUST 要求显式操作币种；写入事实的 currency 为该操作币种，并只更新对应口袋。
...
- **FR-009**: 账单导入 MUST 仅按 mapping 解析出的账户名定位账户；行币种 MUST 写入该账户对应事实与余额口袋。
...
- **FR-012**: 跨账户转账 MUST 用账户名定位双方；用 from/to 操作币种指定口袋；跨币种 MUST 要求 to-amount；同币种可忽略多余 to-amount。
...
- **FR-016**: 财富侧现金估值/校准身份 MUST 能区分同一账户下的不同币种，避免多币种 checkin 互相覆盖。
...
- **Account**: workspace 内名称唯一的账户实体；含 type、active 等账户级属性；**不含**唯一币种身份字段。
- **Currency Balance Pocket**: 账户在某一币种上的余额视图（可由正式事实确定性投影/快照重建，不是第二套账户身份）。
...
- **Valuation Observation (cash)**: 身份需绑定账户与币种（或等价），以支持多币种边界校准。
...
- 自动合并同名但不同 type 的账户。
- 投资组合交易语义大重构（buy/sell/swap 产品行为以现有为准；仅消除账户唯一币种身份带来的冲突）。
...
- 用户已确认方案 A：账户去币种化；余额多币种口袋；无主币种。
- 用户已确认：不做兼容层；实现后一次性迁移现有数据即可；开发数据可丢弃但迁移必须可审计、失败关闭。
- 004 mapping import + open currency 行为保留，本 feature 只改账户身份与币种口袋挂载方式。
- 快照/投影仍可按 `账户类型 → 账户名 → 币种 → 金额` 表达多币种余额（产品层只需保证可读与可重建，不规定存储引擎细节）。
- security/crypto：账户展示名唯一约束保留；账户级唯一 currency 身份移除后，投资事件与持仓继续自带 currency/ticker 信息；本 feature 不重做投资产品语义。
...
| 现金多币种估值身份 | 按账户+币种可区分 | 同左 | checkin 不互相覆盖 |
