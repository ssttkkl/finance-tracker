...
1. **计价币种估值**：各持仓按各自计价（行情）货币估值，**不**折算；
2. **指定币种汇总估值**：各持仓先按计价币种估值，再统一折算为用户传入的展示货币。
...
- 以 **组合持仓市值** 为 **P0 用户可见交付**（计价币种模式 + 可选指定展示币种折算）；
...
### User Story 1 - 组合持仓：计价币种市值与状态 (Priority: P0) 🎯 主交付

作为多市场投资者，我打开持仓/组合视图时，希望每一笔非零持仓都通过 **统一估值** 得到 **计价币种单价、计价币种市值与数据状态**（complete/stale/partial/unsupported），现金持仓单价为 1，这样我能按美元/人民币/港币等 **各自计价货币** 理解仓位，而不是静默空价或假 0。

**Why this priority**: 本 feature 的产品主目标；原子 quote 若不同时接到组合路径，用户仍看不到状态语义。

**Independent Test**: Fake 估值 + 含 USD 美股、CNY/A 股或现金、HKD 港股与不可识别 ticker 的快照；`get_portfolio`（或等价）每条持仓带 `quote_status`；成功项计价币种价格/市值正确；失败项无虚构市价。

**Acceptance Scenarios**:

1. **Given** 组合含现金持仓与可估值证券，**When** 以计价币种模式查询组合（不传展示币种，或明确 `display_currency=None`），**Then** 现金 `unit_price=1`、status=`complete`；证券计价币种单价与市值来自统一估值；每条持仓暴露 `quote_status`（及可选 reason）。
2. **Given** 持仓分属不同计价货币（如美股 USD、港股 HKD、A 股 CNY），**When** 计价币种模式查询，**Then** 各持仓 `quote_currency` / 市值币种保持其计价货币，**不**被折成单一币种。
...
作为用户，我希望传入一个 **展示货币**（如 CNY），系统在 **计价币种估值成功** 的持仓上，用可审计的只读汇率将市值折成该货币，这样我能用单一数字比较全球仓位；折算失败时不得假装已折算。

**Why this priority**: 与 US1 同级产品能力；「分别按计价币种」与「统一指定币种」是同一主路径的两种模式。

**Independent Test**: Fake 估值 + Fake FX；传入 `display_currency=CNY`；断言计价币种字段仍保留，并增加折算市值/汇率字段；FX 缺失时折算市值为空且 status/reason 可区分（不得用 1:1 默默折算异币）。

**Acceptance Scenarios**:

1. **Given** 计价币种估值 complete/stale 且提供展示货币 C，**When** 查询组合，**Then** 对每条有计价币种市值的持仓：若计价货币=C，折算市值=计价币种市值、汇率=1；若计价货币≠C，使用只读 mid 汇率将计价币种市值转为 C（精确十进制）。
2. **Given** 计价币种估值成功但 FX 不可用，**When** 查询，**Then** 计价币种价格/市值/状态仍返回；**折算市值缺失**；不得用错误汇率或 0 填充；对该项给出可区分 reason（如 `fx_unavailable`），组合级可汇总 partial。
3. **Given** 计价币种估值为 partial/unsupported，**When** 指定展示货币，**Then** 不进行折算，折算市值亦缺失。
4. **Given** 非法展示货币（非 3 字母等），**When** 查询，**Then** fail-closed（可操作错误），不返回部分假折算结果。
5. **Given** 同一假源与假 FX，**When** 在 SQLite 与 PostgreSQL 运行时查询，**Then** 计价币种与折算结果语义一致。
...
4. **Given** 合法数量且有单价，**When** `quote`，**Then** 市值=单价×数量（计价币种）。
...
- 预测市场 outcome 价：计价币种语义为合约报价单位；折算到法币时 **仅当** 合同定义了其 `quote_currency`（v1 冻结为 USD 名义）且 FX 可得；否则可 partial 于折算结果。
...
- **PositionValuation**: 单持仓计价币种结果 + 可选展示币种折算（市值、汇率、fx_status/reason）。
- **PortfolioValuationQuery**: 可选 `display_currency`；默认计价币种模式。
