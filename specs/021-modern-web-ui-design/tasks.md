...
- [ ] T028.4 使用 gstack `qa` 完成浏览器 QA；如技能因未提交 worktree 阻断，准确记录原因、风险与补跑条件。此前因其强制要求干净 worktree 而阻断；本任务未获提交或暂存授权。当前自动化浏览器、预览与视觉回归已通过，但仍缺少独立 gstack QA 证据。
- [X] T028.5 运行 `git diff --check`、禁止路径审计及必要前端测试，在 `quickstart.md` 记录最终状态。
