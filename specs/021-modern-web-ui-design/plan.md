...
在不改变 `020-cash-ledger-browser-web` 的数据读取、筛选、稳定分页、投影版本处理、证据详情和可访问性合同的前提下，重构 Web 展示层为技术感、工具化的收支账本工作台。实现使用 Hallmark 的 modern-minimal / Workbench / Cobalt 适配方向：冷色浅表面、单一钴蓝操作强调、现有中文正文与等宽数值字体、精确边界和高密度审阅结构。
...
**性能目标**：不降低 020 已有“100 条投影在 2 分钟内完成筛选、连续翻页和证据详情查看”的验收；不新增网络请求、运行时依赖或后台任务。
...
**阻断前置条件**：候选基线为 `3e1e4b3`，但 020 在当前工作树仍有未提交内容。只有将 020 的 artifacts 和实现收敛为明确提交后，才可从该提交创建 021 implementer worktree。交接时将实际 SHA 写入本计划和 `tasks.md`。
...
CashLedgerPage（保留状态、请求取消、游标、版本更新、焦点恢复）
       ...
       ├── CashFiltersBar（字段与 onChange 合同不变，展示结构重组）
       ├── CashTable（8 列语义和查看入口不变，宽屏/卡片层级重组）
       ├── StatusView / Pagination（事件与文案语义不变，视觉统一）
...
2. **合同回归测试**：先扩展 Vitest 与 E2E，锁定 API 调用、筛选值、8 列顺序、备注位置、无关系摘要、分页、投影更新、错误码文案、证据焦点和表头语义。
3. **视觉失败测试**：先创建固定 fixture 的多视口截图断言，覆盖主流程、加载、空、错误、并列详情与全屏详情；首次生成基线必须人工批准。
