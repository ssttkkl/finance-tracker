...
**性能目标**：不降低 020 已有“100 条收支记录在 2 分钟内完成筛选、自动或手动加载更多和证据详情查看”的验收；不新增网络请求、运行时依赖或后台任务。
...
`CashLedgerPage` 保留既有账户读取、证据详情、`AbortController`、递增请求代次、筛选变更与 `projection.updated` 处理。列表状态从单页和 cursor 栈演进为累计记录、下一个 cursor、首批加载、追加加载和追加错误：

```text
首批成功：替换累计记录，保存 next_cursor
追加成功：按服务端排序追加，并按 projection_id 去重
追加失败：保留已加载记录，显示文字错误与人工重试
筛选变化/版本更新：取消请求，递增代次，清空累计记录和 cursor，从 null cursor 重读首批
```

列表末端的 sentinel 使用 `IntersectionObserver` 预加载下一批；observer 与“加载更多”按钮共用防重入 `loadMore()`。同一 cursor 在任意时刻最多一个请求；`hasMore=false`、追加中、追加错误和组件卸载时不得自动触发。浏览器不支持 observer、键盘/辅助技术使用者或自动加载失败时，始终保留“加载更多”按钮。移除上一页/下一页，不向使用者暴露 cursor。
...
3. **视觉失败测试**：先创建固定 fixture 的多视口截图断言，覆盖默认/展开筛选、首批加载、追加加载、追加失败、全部加载完、空、错误、并列详情与全屏详情；首次生成基线由实施者对照已批准展示层范围复核。
