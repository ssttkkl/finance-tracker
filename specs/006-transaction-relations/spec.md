...
- Q: main 分支配对时间窗与规则族如何落到 v1？ → A: 业务窗口与信号作为规格级验收边界固定如下（实现可命名 rule_id，但不得放宽）：`payment_mirror` 强匹配优先 ≤10 秒且跨源文本交叉，同日弱匹配仅可 pending；`transfer_pair` 同币种强匹配 ≤10 秒且转账信号唯一，同日银联/无卡付现金调拨仅在强信号且唯一时 auto-accept；`credit_repayment` 同币种 cash→loan ≤600 秒，跨币种 ≤10 秒且记录双方金额/币种；`refund_offset` 候选窗口 ≤30 天、auto-accept 默认 ≤14 天（订单/交易号锁定可放宽到 30 天内），退款不早于消费。
- Q: Review Inbox 的 ignore/later 是什么状态？ → A: 不是独立终态；候选保持 `pending_review`，可附加用户“稍后处理”意图/时间戳，继续不影响报表，仍出现在审查列表，直至 accept、reject 或被 supersede。
- Q: 现有 CashTransaction 上的 `offset_*` / `proposed_action` / `transfer_account` 字段如何处理？ → A: 本 feature 的权威关系语义在独立关系对象与投影层；既有行内 offset/transfer 字段不得作为新的权威核销/配对结果，也不得被关系检查改写以表达 accepted 关系。历史残留字段最多只读兼容；新导入路径不得再依赖它们完成核销或转账标记。
- Q: 多条支付平台/银行事实 mirror 时如何计次？ → A: accepted `payment_mirror` 可形成连通组；组内只计一次外部消费，且必须有确定性 canonical；若无法确定唯一 canonical 则 pending。
...
1. **Given** 支付宝事实：2026-06-13 23:15，麦当劳，-30.00 CNY，付款方式含建行卡尾号 1234；银行事实：同日/接近时间，-30.00 CNY，卡尾号 1234，文本含支付通道或商户交叉，**When** 两笔事实均已入库并完成关系检查，**Then** 两条事实都保留，并存在 `payment_mirror` 关系；高置信时 status=`accepted`，弱匹配时 status=`pending_review`。
2. **Given** 已 accepted 的 `payment_mirror` 关系，**When** 计算消费报表，**Then** 该外部消费只计入一次；余额类投影仍保留双方账户的真实流水影响。
3. **Given** 同一候选关系检查已执行过，**When** 对同一批新增事实重复执行关系检查，**Then** 不重复创建相同 accepted 关系或相同 pending 候选（幂等）。
4. **Given** 匹配信号不足（如仅同日同额、无卡尾号、多个候选），**When** 关系检查完成，**Then** 创建 `pending_review` 候选，不自动 accepted，且不影响报表。
5. **Given** 同币种跨平台消费两侧除金额外均为强匹配，但 Decimal 金额存在任意非零差额（例如 30.00 与 30.01 CNY），**When** 关系检查完成，**Then** 可创建含精确 `amount_delta` 的 pending 候选，但不得自动 accepted。
6. **Given** 支付宝与银行两侧金额、币种、卡尾号一致，且有效时间差 ≤10 秒、counterparty/description 交叉验证通过、候选唯一，**When** 关系检查完成，**Then** 可自动 accepted；若仅同日、无 ≤10 秒接近或无卡尾号交叉，**Then** 最多 pending，不得 auto-accept。
7. **Given** 同一外部消费在支付宝、微信、银行各留下事实并形成 accepted `payment_mirror` 连通组，**When** 计算消费报表，**Then** 外部消费只计一次，组内有确定性 canonical；若多条路径导致 canonical 冲突，**Then** 冲突路径 pending 且不双计也不静默任选。
...
- **FR-016**: 跨平台镜像匹配信号 MUST 参考 main 分支跨源去重语义，包括但不限于：金额、币种、时间窗口、账户或卡尾号关联、counterparty/description 交叉、支付平台优先作为 canonical 的规则倾向。同币种两侧 Decimal 金额严格相等是 auto-accept 的必要条件；任意非零金额差额 MUST NOT auto-accept，但 MAY 以精确 `amount_delta` 证据进入 pending。强匹配 auto-accept 还要求：有效时间差 ≤10 秒、跨源文本交叉或卡尾号关联、候选唯一；仅同日同额或缺少卡尾号/交叉验证的匹配 MUST `pending_review`。其他弱匹配或非唯一候选同样必须 pending。
...
- **FR-025**: 对每个种子事实，System MUST 在同一 workspace 的全部既有**活跃**正式事实中，按关系类型使用有界、可索引的候选条件（金额、币种、时间窗口、账户类型、支付方式、卡尾号、别名、文本、source 组合、外部 id 等）筛选候选；候选 MAY 来自任意既有导入批次。增量检查 MUST 只创建或重评至少包含一个种子事实的关系，不得以全库无界扫描作为正确性前置条件。
...
- **SC-002**: 同一跨平台消费导入支付平台与银行两份账单后，系统中存在 2 条正式事实 + 1 条 accepted 或 pending `payment_mirror` 关系（不是删除其中一条，也不是 `duplicate_of`）。
...
- **SC-017**: 对已逻辑删除事实作为唯一候选端点的样本，关系检查 100% 不为其新建 accepted 关系。
...
- 规格级时间窗边界已固定：`payment_mirror` 强匹配 ≤10 秒；`transfer_pair` 同币种强匹配 ≤10 秒；同日银联现金调拨仅限指定强信号；`credit_repayment` 同币种 ≤600 秒、跨币种 ≤10 秒；`refund_offset` 候选 ≤30 天、默认 auto-accept ≤14 天（订单锁定可到 30 天）。
