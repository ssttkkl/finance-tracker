...
export function AccessApp() {
  ...
  return <>{state.active_workspace_id ? <App sidebarFooter={footer} workspacePage={workspacePage} workspaceManagementActive={route === "members"} onWorkspaceManagement={() => setRoute("members")} /> : <Create onSession={setState} onBack={() => setRoute("ledger")} />}</>;
}
