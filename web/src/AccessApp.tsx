...
export function AccessApp() {
  ...
  if (route === "members") { const active = state.workspaces.find(w => w.id === state.active_workspace_id); return <Members activeRole={active?.role ?? "viewer"} onSession={setState} onBack={() => setRoute("ledger")} />; }
  const active = state.workspaces.find(w => w.id === state.active_workspace_id);
  const footer = <div className="sidebar-footer"><div className="workspace-panel"><span>{state.user.email}</span><select aria-label="当前工作区" value={state.active_workspace_id ?? ""} onChange={async e => { if (e.target.value === "__create__") { setRoute("create"); return; } setState(await access.selectWorkspace(e.target.value)); }}>{state.workspaces.map(w => <option key={w.id} value={w.id}>{w.name} · {access.roleLabel[w.role]}</option>)}<option value="__create__">＋ 创建工作区</option></select><button type="button" onClick={() => setRoute("members")}><Icon name="users" />工作区管理</button><button type="button" onClick={async () => { await access.logout(); setState(null); }}><Icon name="logout" />退出登录</button></div></div>;
  return <>{state.active_workspace_id ? <App sidebarFooter={footer} /> : <Create onSession={setState} onBack={() => setRoute("ledger")} />}</>;
}
