...
function Auth({ onSession }: { onSession: (value: Session) => void }) {
  ...
  return <main className="access-centered" data-testid={semanticIds.authScreen}><section className="access-card"><p className="access-eyebrow">{copy.auth.eyebrow}</p><h1>{registering ? copy.auth.registerTitle : copy.auth.loginTitle}</h1><form className="access-form" onSubmit={submit}><label>{copy.auth.email}<input data-testid={semanticIds.authEmail} required name="email" type="email" autoComplete="email" /></label><label>{copy.auth.password}<input data-testid={semanticIds.authPassword} required minLength={12} name="password" type="password" autoComplete={registering ? "new-password" : "current-password"} /></label><button data-testid={semanticIds.authSubmit} className="button-primary" disabled={loading}>{loading ? copy.auth.processing : registering ? copy.auth.register : copy.auth.login}</button></form>{error && <p className="access-error" role="alert">{error}</p>}<button data-testid={semanticIds.authToggleMode} type="button" className="text-button" onClick={() => { setRegistering(!registering); setError(""); }}>{registering ? copy.auth.switchToLogin : copy.auth.switchToRegister}</button></section></main>;
}
...
function Create({ onSession, onBack, showBack }: { onSession: (value: Session) => void; onBack: () => void; showBack: boolean }) {
  ...
  return <main className="access-page" data-testid={semanticIds.workspaceScreen}>{showBack && <BackButton onClick={onBack} />}<p className="access-eyebrow">{copy.workspace.eyebrow}</p><h1>{copy.workspace.createTitle}</h1><p className="access-muted">{copy.workspace.createDetail}</p><section className="access-panel"><form className="access-form" onSubmit={async e => { e.preventDefault(); setLoading(true); setError(""); try { onSession(await access.createWorkspace(name)); } catch { setError("无法创建工作区，请检查名称后重试。"); } finally { setLoading(false); } }}><label>{copy.workspace.name}<input data-testid={semanticIds.workspaceCreateName} value={name} onChange={e => setName(e.target.value)} required maxLength={255} /></label><button data-testid={semanticIds.workspaceCreate} className="button-primary" disabled={loading}>{loading ? copy.workspace.creating : copy.workspace.create}</button>{error && <p className="access-error" role="alert">{error}</p>}</form></section></main>;
}

function WorkspaceSelectionError({ onRetry }: { onRetry: () => void }) {
  return <main className="access-centered" data-testid={semanticIds.workspaceScreen}><section className="access-card"><p className="access-eyebrow">{copy.auth.eyebrow}</p><h1>{copy.workspace.openErrorTitle}</h1><p className="access-error" role="alert">{copy.workspace.openErrorDetail}</p><button data-testid={semanticIds.workspaceRetry} type="button" className="button-primary" onClick={onRetry}>{copy.common.retry}</button></section></main>;
}
...
function AccessAppContent() {
  ...
  const footer = <div className="sidebar-footer"><div className="workspace-panel"><div className="account-line"><span className="account-avatar" aria-hidden="true">{state.user.email.slice(0, 1).toUpperCase()}</span><span className="account-email">{state.user.email}</span></div><label className="workspace-switcher"><span>{copy.workspace.current}</span><select data-testid={semanticIds.workspaceSwitcher} aria-label={copy.workspace.current} value={state.active_workspace_id ?? ""} onChange={async e => { if (e.target.value === "__create__") { setRoute("create"); return; } setWorkspaceError(""); try { const next = await access.selectWorkspace(e.target.value); if (!next.active_workspace_id) throw new Error("workspace_missing"); const target = workspacePath(next.active_workspace_id, workspaceChildPath(location.pathname) || "/"); history.pushState({}, "", target); setState(next); setRoute(routeForPath(target)); window.dispatchEvent(new PopStateEvent("popstate")); } catch { setWorkspaceError(copy.workspace.switchError); } }}>{state.workspaces.map(w => <option key={w.id} value={w.id}>{w.name} · {access.roleLabel[w.role]}</option>)}<option value="__create__">＋ 创建工作区</option></select></label>{workspaceError && <p className="access-error" role="alert">{workspaceError}</p>}<button type="button" className="logout-link" onClick={async () => { await access.logout(); setState(null); }}><Icon name="logout" />{copy.common.logout}</button></div></div>;
...
export function AccessApp() {
  ...
  return (
    <div data-presentation-layout={presentationLayout}>
      <AccessAppContent />
    </div>
  );
}
