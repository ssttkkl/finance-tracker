...
function cashRecordFromEvidence(record: EvidenceRecord, projection: CashProjection): CashRecord {
  return {
    ...
    cash_granularity: record.cash_granularity,
...
export function CashLedgerPage({ onOpenImport, onModalStateChange, embedded = false }: { onOpenImport?: () => void; onModalStateChange?: (open: boolean) => void; embedded?: boolean } = {}) {
  ...
  const ledgerContent = <section className="ledger ledger-workbench" id="cash-ledger" data-testid={semanticIds.ledgerScreen} aria-label={copy.ledger.title}><header className="page-header" data-testid={semanticIds.ledgerHeader}><div><h1>{copy.ledger.title}</h1></div><div className="page-header-actions"><button data-testid={semanticIds.ledgerAdd} type="button" className="button-secondary" onClick={openNew}>{copy.ledger.create}</button><button data-testid={semanticIds.ledgerImport} type="button" className="button-primary" onClick={onOpenImport ?? (() => setImporting(true))}>{copy.ledger.import}</button></div></header>{accountsError ? <div className="status-view status-error" data-status-kind="error" role="alert"><p>无法读取账户，请稍后重试。</p><button type="button" onClick={loadAccounts}>重试账户</button></div> : null}{batchError ? <div className="batch-error" role="alert">{batchError}</div> : null}<div data-testid={semanticIds.ledgerFilters}><CashFiltersBar filters={filters} accounts={accounts} filterOptions={filterOptions} filterOptionsReady={filterOptionsReady} filterOptionsLoading={filterOptionsLoading} amountFilterState={amountFilterState} onChange={updateFilters} onAmountFilterChange={clearAmountFilterState} /></div>{status === "loading" ? <><div data-testid={semanticIds.ledgerList}><CashTable items={[]} loading onEvidence={openEvidence} onAction={handleRowAction} /></div><StatusView kind="loading" message={projectionUpdated ? copy.ledger.updatedLoading : undefined} /></> : null}{status === "empty" ? <div data-testid={semanticIds.ledgerEmpty}><StatusView kind="empty" /></div> : null}{status === "error" ? <StatusView testID={semanticIds.ledgerRetry} kind="error" message={errorMessage} onRetry={resetAndLoad} /> : null}{status === "ready" ? <><div data-testid={semanticIds.ledgerList}><CashTable items={items} monthlySummaries={monthlySummaries} selectable selectedIds={selectedIds} onToggleSelection={toggleSelection} onToggleAll={toggleAllSelection} onEvidence={openEvidence} onAction={handleRowAction} /></div>{projectionUpdated ? <div className="update-notice" role="status"><p>{copy.ledger.updated}</p><button ref={updateConfirmation} type="button" onClick={confirmUpdatedList}>{copy.ledger.viewUpdated}</button></div> : null}<LoadMoreControl hasMore={Boolean(nextCursor)} loading={appendLoading} error={appendError} onLoadMore={appendError ? retryMore : loadMore} />{selectedIds.size ? <div className="batch-toolbar" role="toolbar" aria-label="批量操作"><span>已选 {selectedIds.size} 项</span><button type="button" className="button-primary" onClick={openBatchClassification}>修改分类</button><button type="button" className="button-danger" onClick={() => void openBatchDelete()} disabled={projectionVersion === null}>删除</button><button type="button" className="button-secondary" onClick={() => setSelectedIds(new Set())}>取消选择</button></div> : null}</> : null}</section>;
