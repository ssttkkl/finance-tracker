...
export function CashTable({ items, monthlySummaries = [], loading = false, onEvidence }: Props) {
  ...
  const tableRows = monthlySummaries.length ? items.flatMap((item) => {
    ...
    return [divider, <tr className="cash-row" data-projection-id={item.projection_id} key={item.projection_id}>
      ...
      <td className="account" data-label="账户" headers="cash-column-account">{accountLabel(item)}</td><td className="counterparty" data-label="交易信息" headers="cash-column-transaction-info"><span className="counterparty-primary">{item.counterparty || "-"}</span><span className="note" data-label="备注">{item.note || "-"}</span></td><td className="economic-type" data-label="经济类型" headers="cash-column-economic-type"><span className="mobile-field-label">经济类型：</span>{economicTypeLabel(item.economic_type)}</td>
  ...
  return <div className="table-wrap"><table className="cash-table">
    ...
    <thead className="table-head"><tr><th id="cash-column-occurred-at" scope="col">发生时间</th><th id="cash-column-account" scope="col">账户</th><th id="cash-column-transaction-info" scope="col">交易信息</th><th id="cash-column-economic-type" scope="col">经济类型</th><th id="cash-column-amount" scope="col" className="amount">金额</th><th id="cash-column-action" scope="col"><span className="sr-only">操作</span></th></tr></thead>
    <tbody>{loading ? Array.from({ length: 3 }, (_value, index) => <tr className="loading-row" data-testid="现金流水骨架行" key={index}>
      {Array.from({ length: 6 }, (_cell, cellIndex) => <td key={cellIndex}><span className="skeleton-cell" aria-hidden="true" /></td>)}
    </tr>) : tableRows}</tbody>
  </table></div>;
}
