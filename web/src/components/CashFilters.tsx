...
export function CashFiltersBar({ filters, accounts, filterOptions, filterOptionsReady, filterOptionsLoading, amountFilterState, onChange, onAmountFilterChange }: Props) {
  ...
  return <details className="filters" aria-label="账本筛选工具" data-layout="filter-grid">
    ...
    <div className="filter-grid">
      ...
      <label>分类<select aria-label="分类" value={filters.category ?? ""} disabled={filterOptionsLoading || !filterOptionsReady} onChange={(e) => update("category", e.target.value)}><option value="">全部分类</option>{[...new Set([filters.category, ...filterOptions.categories].filter(Boolean) as string[])].map((category) => <option value={category} key={category}>{category}</option>)}</select></label>
