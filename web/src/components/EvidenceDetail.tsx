...
export function EvidenceDetail({ evidence, loading, error, onClose, onRetry }: Props) {
  ...
  return <aside ref={dialog} className="evidence" role="dialog" aria-modal="true" aria-label="证据详情">
    ...
    {root ? <div className="evidence-content">
      <h3>投影结果</h3><dl><dt>投影标识</dt><dd className="mono">{evidence.projection.projection_id}</dd><dt>净额</dt><dd className="mono">{evidence.projection.amount} {evidence.projection.currency}</dd><dt>经济类型</dt><dd>{evidence.projection.economic_type === "expense" ? "消费" : evidence.projection.economic_type === "income" ? "收入" : "内部转账"}</dd></dl>
      <h3>主记录</h3><dl><dt>发生时间</dt><dd className="mono">{formatOccurredAt(root.occurred_at)}</dd><dt>账户</dt><dd>{root.account.name}</dd><dt>交易对方</dt><dd>{root.counterparty || "未提供"}</dd><dt>分类</dt><dd>{root.category || "未分类"}</dd><dt>备注</dt><dd>{root.note || "未提供"}</dd><dt>金额</dt><dd className="mono">{root.amount} {root.currency}</dd><dt>导入渠道</dt><dd>{root.source_type || "未提供"}</dd><dt>业务行标识</dt><dd className="mono">{root.record_id || "未提供"}</dd></dl>
      <h3>来源行快照</h3>{snapshotEntries(root.source_snapshot)}
      <h3>成员流水</h3>{evidence.members.length ? <ul>{evidence.members.map((member) => <li key={member.id}>{formatOccurredAt(member.occurred_at)}，{member.account.name}，{member.amount} {member.currency}（{member.roles.map((role) => memberRoles[role] ?? "未识别角色").join("、")}）</li>)}</ul> : <p>此投影未提供成员流水。</p>}
      <h3>已采用关系</h3>{evidence.accepted_relations.length ? <ul>{evidence.accepted_relations.map((relation) => <li key={relation.id}>{relationKinds[relation.kind] ?? "未识别的关系类型"}{relation.rule_id ? `（${relation.rule_id}）` : ""}</li>)}</ul> : <p>此投影没有已采用关系。</p>}
      <h3>未生效关系提示</h3>{evidence.inactive_relation_hints.length ? <ul>{evidence.inactive_relation_hints.map((relation) => <li key={relation.id}>{relationKinds[relation.kind] ?? "未识别的关系类型"}：{relationStatuses[relation.status]}。{recordSummary(relation.primary_record)}；{recordSummary(relation.secondary_record)}</li>)}</ul> : <p>此投影没有未生效关系提示。</p>}
      <h3>退款时间线</h3>{evidence.refund_timeline.length ? <ul>{evidence.refund_timeline.map((refund) => <li key={refund.record_id}>{formatOccurredAt(refund.occurred_at)}，{refund.amount} {refund.currency}，{refund.source_type || "未提供"}</li>)}</ul> : <p>此投影没有退款。</p>}
    </div> : null}
  </aside>;
}
