...
const cashDisplayNames: Record<string, string> = {
  cny: "人民币", eur: "欧元", gbp: "英镑", hkd: "港币", jpy: "日元", usd: "美元",
};

function holdingDisplayName(position: PortfolioPosition) {
  if (position.display_name?.trim()) return position.display_name.trim();
  if (position.is_cash) return cashDisplayNames[position.ticker.toLowerCase()] ?? position.ticker.toUpperCase();
  return position.ticker.toUpperCase();
}

function holdingCodeLine(position: PortfolioPosition, accountName: string, merged: boolean) {
  return `${position.ticker} · ${merged ? "多个账户" : accountName}`;
}
...
export function InvestmentHoldings({ portfolio, accounts, options, loading, refreshing, error, onOptionsChange, onRetry }: Props) {
  ...
  return <section className="investment-section holdings-section" aria-labelledby="holdings-title">
    ...
    {!loading && portfolio && (cashRows.length || rows.length) ? <div className="holdings-table-wrap"><table className="holdings-table" aria-label="当前持仓"><caption className="sr-only">当前持仓</caption><thead><tr><th scope="col">标的 / 账户</th><th scope="col">当前单价 / 平均成本</th><th scope="col">数量</th><th scope="col">当前市值</th><th scope="col">仓位</th><th scope="col">浮盈亏</th><th scope="col">浮盈亏率</th><th scope="col">{periodLabels[options.period]}盈亏</th><th scope="col">{periodLabels[options.period]}盈亏率</th></tr></thead><tbody>{cashRows.map(({ accountName, position }) => {
      ...
      return <tr className="holding-cash-row" key={`${accountName}:${position.ticker}:${position.cost_currency}`}><td className="holding-symbol" data-label="标的 / 账户"><strong>{holdingDisplayName(position)}</strong><small>{holdingCodeLine(position, accountName, false)}</small></td><td className="holding-cash-gap" colSpan={2} aria-hidden="true" /><td className="mono holding-market holding-cash-market" data-label="当前市值">{value === null || value === undefined ? "—" : `${displayValue(value)} ${rowCurrency}`}</td><td className="holding-cash-tail" colSpan={5} aria-hidden="true" /></tr>;
      ...
      const rawAverageCost = decimalDivide(position.total_cost, position.shares);
      const averageCost = currency && rawAverageCost !== null
        ? decimalMultiply(rawAverageCost, position.fx_rate)
        : rawAverageCost;
      const priceCurrency = currency || position.quote_currency || "";
      const averageCostCurrency = currency || position.cost_currency;
      ...
      return <tr key={`${accountName}:${position.ticker}:${position.cost_currency}`}><td className="holding-symbol" data-label="标的 / 账户"><strong>{holdingDisplayName(position)}</strong><small>{holdingCodeLine(position, accountName, merged)}</small></td><td className="mono holding-price" data-label="当前单价 / 平均成本"><div className="holding-price-current-line"><span className="holding-price-mobile-label">现价</span><span className={`holding-price-current ${currentPriceTone}`}>{currentPrice === null ? "—" : `${displayValue(currentPrice)} ${priceCurrency}`}</span>{quoteSession ? <span className="holding-price-session">{quoteSession}</span> : null}</div><div className="holding-price-average"><span className="holding-price-mobile-label">成本价</span><span>{averageCost === null ? "—" : `${displayValue(averageCost)} ${averageCostCurrency}`}</span></div><small className="holding-quote-meta">{formatQuoteTime(position.quote_observed_at)}</small></td><td className="mono" data-label="数量">{displayValue(position.shares)}</td><td className="mono holding-market" data-label="当前市值">{value === null || value === undefined ? "—" : `${displayValue(value)} ${rowCurrency}`}</td><td className="mono holding-position" data-label="仓位">{percentage(weight)}</td><td className={`mono holding-pnl ${decimalSign(displayedProfit) !== null && decimalSign(displayedProfit)! < 0 ? "negative" : "positive"}`} data-label="浮盈亏">{signedValue(displayedProfit, pnlCurrency)}</td><td className={`mono holding-pnl-rate ${decimalSign(rate) !== null && decimalSign(rate)! < 0 ? "negative" : "positive"}`} data-label="浮盈亏率">{percentage(rate)}</td><td className={`mono holding-period ${decimalSign(displayedPeriod) !== null && decimalSign(displayedPeriod)! < 0 ? "negative" : "positive"}`} data-label={`${periodLabels[options.period]}盈亏`}><span>{signedValue(displayedPeriod, rowCurrency)}</span>{positionBaselineLabel ? <small className="holding-period-note">{positionBaselineLabel}</small> : null}</td><td className={`mono holding-period-rate ${decimalSign(position.period_profit_rate) !== null && decimalSign(position.period_profit_rate)! < 0 ? "negative" : "positive"}`} data-label={`${periodLabels[options.period]}盈亏率`}>{percentage(position.period_profit_rate)}</td></tr>;
