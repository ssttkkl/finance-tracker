      ...
      <div className="evidence-content">
        ...
        <section className="evidence-section record-form-section" data-testid={semanticIds.recordScreen} aria-label={copy.record.editTitle}>
          ...
          <div className="edit-fields">
            ...
            <div data-testid={semanticIds.recordAllocation} className="edit-row component-editor-row"><span className="edit-field-label"><UiIcon name="account" /><span>{copy.record.allocation}</span></span><div className="component-editor" aria-label={copy.record.allocation}>{components.map((component, index) => <div className="component-editor-item" key={`${index}-${component.account_name}`}><select data-testid={index === 0 ? semanticIds.recordAccount : undefined} aria-label={index === 0 ? copy.record.account : `第${index + 1}个账户`} value={component.account_name} onChange={(event) => updateComponent(index, "account_name", event.target.value)}><option value="">选择账户</option>{accounts.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select><input className="mono" inputMode="decimal" aria-label={`第${index + 1}个${copy.record.allocation}`} value={component.amount} onChange={(event) => updateComponent(index, "amount", event.target.value)} /><button type="button" className="icon-only-button icon-quiet-button" aria-label={copy.record.allocationRemove} title={copy.record.allocationRemove} disabled={components.length <= 1} onClick={() => removeComponent(index)}><UiIcon name="x" /></button></div>)}<button type="button" className="text-button component-add-button" onClick={addComponent}>{copy.record.allocationAdd}</button><p className={`component-total ${allocationMatches ? "is-complete" : "is-incomplete"}`} role="status">{allocationMatches ? copy.record.allocationMatch : copy.record.allocationIncomplete}</p></div></div>
