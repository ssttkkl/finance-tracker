...
describe("统一账本外壳", () => {
  it("在收支账本和投资账本路由下保持同一棵分级侧边导航", async () => {
    ...
    expect(within(navigation).getAllByRole("link").map((link) => link.textContent)).toEqual(["收支账本", "投资账本", "当前持仓", "投资事件"]);
    ...
    expect(within(investmentNavigation).getAllByRole("link").map((link) => link.textContent)).toEqual(["收支账本", "投资账本", "当前持仓", "投资事件"]);
