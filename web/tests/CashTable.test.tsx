...
it("空的交易对方和备注显示横杠", () => {
  render(<CashTable items={[{ ...projection("empty"), counterparty: "", note: "" }]} onEvidence={() => undefined} />);
...
it("普通收入和支出都显示货币单位", () => {
  ...
  render(<CashTable items={[projection("expense", "payment_mirror"), income]} onEvidence={() => undefined} />);
...
it("按月份插入收入和支出汇总分割行，并按币种展示", () => {
  ...
  render(<CashTable items={[projection("current", "payment_mirror"), older]} monthlySummaries={monthlySummaries} onEvidence={() => undefined} />);
...
it("同币种内部转账只显示一次金额且不显示负号", () => {
  render(<CashTable items={[transfer()]} onEvidence={() => undefined} />);
...
it("跨币种内部转账显示两端金额且不显示负号", () => {
  render(<CashTable items={[transfer(true)]} onEvidence={() => undefined} />);
