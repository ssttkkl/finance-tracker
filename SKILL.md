...
| **yfinance ticker 格式不兼容 ft 的存储格式** | 2026-06-28 修复：`_fetch_prices()` 前新增 `_normalize_ticker()`：`avgo.us`→`AVGO`、`00700.hk`→`0700.HK`（4位数HK ticker前补零）、`.SZ/.SS` 保留不变。注意 yfinance 对 HK 股票批量下载有时会失败，单独拉正常。 |
