...
**历史回填与 worktree 合并陷阱：** `do_append()` 校验“既有 records + 新增 rows”时，必须使用与 `_replay_security_csv()` 完全相同的**全局、稳定、按 `date` 排序**规则；不能按文件收集顺序重放。否则补录较早成交（例如 USD 建仓 → 零股 CHECKIN 平仓 → 后续 CNY 建仓）会错误触发成本币种冲突。明确同时间戳的 tie-break：先保留既有日文件内的行序，再在其后追加本次导入行序，并为该历史回填场景做 append→repair→verify 回归测试。若在隔离 worktree 实现后合并回已有未提交改动的分支，先记录并确认这些改动来源；冲突不能只选一边。解决后必须跑**合并后的**完整 `PYTHONPATH=src pytest -q`，因为新 replay/校验实现很容易覆盖同时进行的 `stock list` 展示契约（动态现金 ticker 不报价、非本账户货币为 N/A、空头负市值、多币种不合并）。
...
| **ICBC 信用卡 PDF 内含多卡交易 — 用 card_number 路由分流** | 工行信用卡历史明细 PDF 可能包含**多张卡**的交易记录（例如主卡和附卡混在同一份 PDF 中；完整卡号必须脱敏为 `******`）。转换器已提取 `card_number`（卡号后4位）并尝试按 `{bill_type}_{card_num}` 匹配 mapping 规则（见 `do_convert` 第1133行）。例如卡号 1200 的规则是 `source: icbc_credit_1200`，0851 的规则是 `source: icbc_credit_0851`。**mapping.yaml 中需为每张卡分别添加规则**，否则全部走通用的 `icbc_credit` fallback 路由到同一账户。检测方法：`pdftotext PDF -upw PASS - | grep -oE '\d{16}' | sort -u` 可列出 PDF 中所有卡号。 |
