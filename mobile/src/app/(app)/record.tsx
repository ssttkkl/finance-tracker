  ...
  useEffect(() => {
    ...
    if (creating) {
      ...
      Promise.all([client.fetchEvidence(projectionId), client.fetchCashAccounts(), client.fetchLedgerOptions(), client.fetchCashCategories()]).then(([nextEvidence, nextAccounts, nextOptions, nextCategories]) => {
        ...
        setAccountId(record.account?.id ?? null);
...
const styles = StyleSheet.create({
  ...
  componentRow: { flexDirection: "row", justifyContent: "space-between", gap: 12, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: nativeColors.rule },
});
