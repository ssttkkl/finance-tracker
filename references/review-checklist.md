...
- **不计收支方向检测**：支付宝 `方向=不计收支` 的记录需按 `交易状态` 列分三种情况：① **交易关闭** → 下单未付款，无实际资金流动，正确跳过（代码已处理，审查确认无漏网）；② **退款成功** → 真实退款，应进入 `_pair_refunds` 配对核销；③ **交易成功/转出成功** → 余额宝收益/基金买入等，保留为 income。注意检查转换后 CSV 中是否有描述为商品名、对面为脱敏店铺名（`ap**店`/`x***4`）的正数条目 → 这些是交易关闭应跳过的，不应出现在主 CSV 中
- 部分退款净额在 CSV 中缺失 → 补加
...
- **漏删检查**：检查 merged 中是否有同来源+同日+同金额+同counterparty的明显重复行。
  - **注意**：同一个账户内的多笔相同记录不视为重复。银行源数据中同一笔扣款可能因银行记账规则产生多行（如授权+结算、补扣管理费逐笔列出、退货退款前后两条）。这些是银行的"多笔相同记录"，不是需要去重的"重复行"。
  - 去重只针对**支付宝/微信和银行卡之间**的跨来源相同交易，不检查单账户内的多次出现。
...
### 支付宝转换常见修复
- **下单未付款（交易关闭）跳过**：`_read_alipay_raw` 中判断 `direction == "不计收支" and txn_status == "交易关闭"` → `continue`。怀疑漏跳时检查原始支付宝 CSV 的"交易状态"列
- **不计收支+交易关闭条目出现在主 CSV**：说明 skip 逻辑遗漏，检查 `txn_status` 变量是否从正确列读取（列名`交易状态`），以及是否有其他方向=不计收支的非退款条目
