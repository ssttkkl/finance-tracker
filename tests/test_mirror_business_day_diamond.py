...
def _fv(id, amount, *, account, bill_source=None, note="", counterparty="",
    ...
    if bill_source is None:
        if fid.startswith("p") or "alipay" in fid or "wechat" in fid:
            ...
            bill_source = "icbc"
    src = source if source is not None else bill_source
    return FactView(
        id=id,
