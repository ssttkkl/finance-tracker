...
def _add_cash(
    ...
    session.flush()
    row._test_component_id = component.id
    return row
...
def test_unique_strong_candidate_is_idempotently_confirmed_and_projects_as_bank_security_transfer(funding_runtime):
    ...
    assert relation["cash_transaction_component_id"] == cash._test_component_id
...
def test_institution_name_prefers_the_unique_exact_candidate_over_generic_transfer(funding_runtime):
    ...
    assert relations[0]["cash_transaction_component_id"] == institution_cash._test_component_id
...
def test_institution_name_candidate_upgrades_unreviewed_system_candidate(funding_runtime):
    ...
    with sessions.begin() as session:
        ...
        session.add(CashInvestmentFundingRelationModel(
            workspace_id="funding", cash_transaction_component_id=cash._test_component_id, investment_event_id=investment.id,
...
def test_stronger_institution_candidate_archives_legacy_generic_candidate(funding_runtime):
    ...
    with sessions.begin() as session:
        ...
        session.add_all([
            CashInvestmentFundingRelationModel(
                workspace_id="funding", cash_transaction_component_id=generic_cash._test_component_id,
                ...
                CashInvestmentFundingRelationModel(
                    workspace_id="funding", cash_transaction_component_id=institution_cash._test_component_id,
                investment_event_id=investment.id, direction="cash_to_investment",
                status="pending_review", rule_id="cash-investment-funding-v1", created_by="system",
                evidence={"business_day_window": 0, "candidate_count": 2},
            ),
    ...
    by_cash_id = {relation["cash_transaction_component_id"]: relation for relation in results}
    assert by_cash_id[generic_cash._test_component_id]["status"] == "rejected"
    assert by_cash_id[generic_cash._test_component_id]["decision_reason"] == "no_longer_candidate"
    assert by_cash_id[institution_cash._test_component_id]["status"] == "accepted"
...
def test_ordinary_or_ambiguous_candidates_require_manual_decision_and_consume_endpoints_once(funding_runtime):
    ...
    other = next(item for item in candidates if item["cash_transaction_component_id"] != accepted["cash_transaction_component_id"])
    ...
    assert rejected["cash_transaction_component_id"] != first_cash._test_component_id or accepted["cash_transaction_component_id"] == first_cash._test_component_id
    ...
    with pytest.raises(IntegrityError):
        with sessions.begin() as session:
            session.add(CashInvestmentFundingRelationModel(
                ...
                cash_transaction_component_id=cash._test_component_id,
