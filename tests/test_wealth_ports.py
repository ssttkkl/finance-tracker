...
def test_typed_wealth_fact_values_preserve_identity_and_exact_values() -> None:
    ...
    account = AccountFact("w", 1, "cash", {})
    valuation = ValuationFact(
        workspace_id="w", observation_id="obs-1", identity_kind="cash_account",
        identity="1:CNY", observation_kind="boundary_checkin", value=Decimal("1.23"),
        ...
        source_revision="r1", trust="trusted_checkin", owner_account_id=1,
    )
    assert account.account_id == valuation.owner_account_id
    assert valuation.identity == "1:CNY"
