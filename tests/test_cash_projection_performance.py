...
def _seed_cash_projection_workload(sessions) -> None:
    from ft.adapters.relational.models import (
        AccountModel, CashTransactionComponentModel, CashTransactionModel,
    ...
    with sessions.begin() as session:
        ...
        if session.bind.dialect.name == "postgresql":
            ...
            session.execute(text(
                "SELECT setval("
                "pg_get_serial_sequence('cash_transaction_components', 'id'), "
                "COALESCE((SELECT MAX(id) FROM cash_transaction_components), 1), true)"
            ))
