...
def test_scan_alipay_multiple_funding_accounts_are_exposed_as_component_groups():
    from ft.application.statement_account_mapping import scan_source_rows
    ...
    assert [group.source_account_key for group in groups] == ["支付宝余额", "花呗"]


def test_scan_alipay_multiple_funding_accounts_have_no_scan_issue():
    from ft.application.statement_account_mapping import scan_source_rows_with_issues
...
def test_database_mapped_parser_exposes_alipay_components_after_saved_mapping(tmp_path):
    from sqlalchemy import select
    ...
    with unit_of_work(sessions, "composite-workspace") as uow:
        uow.accounts.add_raw({"name": "数据库账户", "type": "cash", "currency": "CNY"})
        uow.accounts.add_raw({"name": "数据库花呗", "type": "loan", "currency": "CNY"})
        uow.commit()
    with sessions() as session:
        account_id = session.scalar(select(AccountModel.id).where(
        ...
        loan_account_id = session.scalar(select(AccountModel.id).where(
            AccountModel.workspace_id == "composite-workspace",
            AccountModel.name == "数据库花呗",
        ))
    with unit_of_work(sessions, "composite-workspace") as uow:
        uow.statement_account_mappings.upsert(
            source_type="alipay", identity_kind="payment_method",
        ...
        uow.statement_account_mappings.upsert(
            source_type="alipay", identity_kind="payment_method",
            source_account_key="花呗", account_id=loan_account_id, confirmed_by="web",
        )
        uow.commit()
    ...
    assert len(rows) == 2
    composite = rows[0]
    assert composite["record_id"] == "ambiguous"
    assert composite["account_name"] == ""
    assert composite["component_account_names"] == {
        "支付宝余额": "数据库账户",
        "花呗": "数据库花呗",
    }
    assert composite["component_allocation"]["status"] == "requires_allocation"
    assert [item["account_name"] for item in composite["components"]] == [
        "数据库账户", "数据库花呗",
    ]
    assert rows[1]["record_id"] == "valid"
    assert rows[1]["account_name"] == "数据库账户"


def test_import_service_blocks_unallocated_composite_and_keeps_unknown_mapping_fail_closed(tmp_path):
