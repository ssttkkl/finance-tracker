...
def _chain_connector(*, malformed=False):
    ...
    return PolymarketConnector(
        credentials={"proxy_wallet": _FUNDER},
        _fetch_fn=lambda _url: [_load_activity_trade()], _rpc_fetch_fn=rpc,
    )
...
class TestPolymarketSyncSQLite:
    ...
    def test_activity_and_pusd_checkin_commit_together_and_preserve_market_position(self, sqlite_pm_sync):
        ...
        second = service.sync(provider="polymarket", account_name="Polymarket", connector=_chain_connector())
        assert second.ok and second.count == 0

    def test_malformed_pusd_balance_rolls_back_activity_and_leaves_cursor_unchanged(self, sqlite_pm_sync):
        service, uow = sqlite_pm_sync
    ...
    def test_successful_empty_chain_scan_still_persists_its_compound_cursor(self, sqlite_pm_sync):
        """An empty page is a successful checkpoint, not permission to lose progress."""
        service, uow = sqlite_pm_sync
        ...
        result = service.sync(provider="polymarket", account_name="Polymarket", connector=EmptyConnector())
        assert result.ok
        with uow as u:
            account_id = service._resolve_account_id(u, "Polymarket")
