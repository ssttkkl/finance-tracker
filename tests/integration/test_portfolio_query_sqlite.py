"""SQLite contract for bounded, read-only portfolio quote queries."""
...
def test_bounded_portfolio_query_preserves_sqlite_holdings_without_writes(tmp_path):
    engine = create_relational_engine(f"sqlite+pysqlite:///{tmp_path / 'portfolio.db'}")
    try:
...
def test_sqlite_portfolio_quote_contract_deduplicates_and_preserves_display_values(tmp_path):
    class BatchProvider:
        def __init__(self):
            self.calls = []
    ...
    engine = create_relational_engine(f"sqlite+pysqlite:///{tmp_path / 'portfolio-contract.db'}")
    try:
        create_schema(engine)
        sessions = create_session_factory(engine)
...
def test_sqlite_portfolio_repository_load_holdings_keeps_names_without_history_payload(tmp_path):
    from ft.adapters.relational.models import InvestmentEventModel

    engine = create_relational_engine(f"sqlite+pysqlite:///{tmp_path / 'portfolio-holdings.db'}")
    try:
        create_schema(engine)
        sessions = create_session_factory(engine)
