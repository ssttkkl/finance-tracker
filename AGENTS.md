...
1. 完成构建后、提交或进入发布准备前，所有代码变更必须运行 gstack `/review`。阻断性 finding 必须修复并重新评审；若 finding 反映规格或方案缺口，先更新对应 Spec Kit artifact。
2. UI/Web 变更在同一开发后验证阶段运行 `/design-review`，开发者体验变更运行 `/devex-review`，安全敏感变更运行 `/cso`；高风险变更或评审结论冲突时使用 `/codex` 获取第二意见。
3. 完成 UI 设计后在同一阶段运行 Hallmark `audit`。`audit` 只输出按影响排序的设计问题，不直接修改；必要修复必须回写为 `tasks.md` 中的任务后再实施。
...
### 6. 开发后验证（二）：行为、真实界面与回归测试

1. 在代码审查后、提交或进入发布准备前，运行受影响测试、完整测试套件、类型检查、lint 和构建中项目实际提供的命令。存储行为变更必须在 SQLite 与真实 PostgreSQL 上运行同一契约矩阵；任一后端缺少证据均不得声明完成。
2. Web 行为或交互变更必须在同一开发后验证阶段运行 gstack `/qa`，覆盖主流程、错误/空状态和相关回归；修复后重新 QA。仅需报告问题且未获修复授权时使用 `/qa-only`。
...
5. 检查最终 diff、未跟踪文件和 tasks 完成状态后，才能声明该提交已完成开发后验证。验证任务从 `[ ]` 改为 `[X]` 时，必须在任务下记录实际执行的命令或 skill、结果、验证的 HEAD、比较基线（适用时）、执行时间和未解决风险；不得以计划、推测或旧结果代替实际证据。当前 HEAD、目标分支基线、依赖或运行环境发生影响验证的变化时，必须将对应验证任务恢复为 `[ ]` 并重新执行。
6. 发布阶段以当前 feature 的 `tasks.md` 为唯一 PR 准入清单：仅当全部适用发布门禁任务均为 `[X]`、证据中的 HEAD 与当前待推送提交一致、比较基线与 PR 目标分支一致，且不存在未完成或未说明理由的“不适用”项时，才可推送、创建或更新 PR。`/ship` 不得另设或重复一套门禁。
