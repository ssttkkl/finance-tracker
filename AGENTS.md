...
5. 检查最终 diff、未跟踪文件和 tasks 完成状态后，才能声明该提交已完成开发后验证。未运行的验证必须明确列出：未验证内容、原因、风险和开发者可执行的准确命令。不得把推测写成通过结论。
6. 发布阶段复用已记录的开发后验证证据；当待推送提交、目标分支基线和运行环境均未改变时，不因创建或更新 PR 重复执行 `/review`、`/qa` 或同一测试。新增提交、目标分支同步、依赖或部署配置变化，或审查发现新风险时，必须从受影响验证项开始重新执行。
